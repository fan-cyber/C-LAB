...
class Summarizer:
    def __init__(self, paper_path, summary_path):
        '''
        param:
            paper_path: 文献的pdf文件夹路径或者单个pdf文件路径
            summary_path: 保存文献总结(markdown)的文件夹路径
        '''
        #################### 将文献库中所有文献保存为Paper类, 并存入paper_list中 ####################
        paper_list = []
        ...
        #################################### 设置粗筛的超参数 ####################################
        self.keyword = "chemistry"                      # 粗筛的关键词
        self.max_token_num = 4096                       # gpt3.5的最大token数
        self.encoding = tiktoken.get_encoding("gpt2")   # 粗筛编码器
    ...
    def summarize(self):
        '''
            对所有文献进行总结, 每个文献的总结保存为一个markdown文件
            Step1: 先用title, abstract, introduction进行全文总结
            Step2: 总结method
            Step3: 总结conclusion
            Step4: 将总结的结果保存为markdown文件
        '''
