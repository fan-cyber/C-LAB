...
class Paper:
    ...
    def get_title(self):
        ...
        for page in self.pdf:
            for block in page.get_text('dict')['blocks']:  # 遍历每个文本块
        ...
        max_font_sizes.sort()
        cur_title = ''
        for page in self.pdf:  # 遍历每一页
            for block in page.get_text('dict')['blocks']:  # 遍历每个文本块
                if block['type'] == 0 and len(block['lines']):  # 如果是文字类型
                    if len(block['lines'][0]['spans']):
                        cur_string = block['lines'][0]['spans'][0]['text']
                        font_size = block['lines'][0]['spans'][0]['size']  # 字体大小
                        if abs(font_size - max_font_sizes[-1]) < 0.3 or abs(font_size - max_font_sizes[-2]) < 0.3:
                            for line_idx in range(1, len(block['lines'])):  # 标题有多行的情况
...
if __name__ == '__main__':
    paper = Paper(path='./NewOne/热稳定Au@Pd晶种合成金纳米双锥.pdf')
