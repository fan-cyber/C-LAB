...
def save_as_markdown(text, save_file):
    """Save a single paper summary as a markdown file

    Args:
        text (str): the summary text
        save_file (str): the path to save the markdown file
    """
...
class SummaryModule:
    ...
    def summarize(self):
        ...
        """
        markdown = []
        for paper in tqdm(self.paper_list):
            print(f"[{paper.title}]")
            ########################################## Step1 ##########################################
    ...
    def section_name_scan(self):
        """Scan the section name of each paper
        """
